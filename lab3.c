 ...
 **/
 ...
 /* Disclaimer - we worked with Dr. Santago to try to fix the one error 
 and he believes the 1 error is from a valgrind issue */
...
/* bubble sort to sort the words by their respective counts */
struct WordFreq ** bubble_sort(struct WordFreq ** wfpp, int size);
/* printing word followed by its corresponding count */
...
*/
int process_characters(char filename[], char output[]){
    /* this will store the current character we are examining in the file */
    char ch;
    ...
    int size;
    /* index will be the position of the current word we are looking at in wfpp */
    int index;
    /* this is a pointer that points to a list of struct pointers */
    ...
    while(ch != EOF) {
        ...
        if (n>=19){
            /* using string buffer to store the current word */
            ...
            /* otherwise we check if the current word has already been processed */
                ...
                /* index will be -1 if the word has not been scene before, otherwise it will contain
                the index of the struct that has the word so we can update count in the correct position */
               ...
               to a list of points that points to struct, with a new struct for the new word we found */
                    ...
                    /*add this word by creating a new struct and adding it to the wfpp pointer */
    ...
    - this method functions the exact same way as inside of the while loop when we have a word and we 
    run a search to see if it has already been seen or not and if so we update the count and otherwise
    we create a new struct and update wfpp */
    if (strlen(buffer) > 0){
        str_buff = (char *) malloc(sizeof(char) * (strlen(buffer)+1));
        str_buff[strlen(buffer)] = '\0';
        strcpy(str_buff, buffer);
    ...
    /* freeing all of the memory we allocated */
    for (i=0;i<size;i++){
...
/* this function returns the index of the struct that matches the current word, but
if the word has not been seen yet it will return -1 indicating that we must create a new
struct for this newly seen word */
...
/* this function prints the words with their respective counts */
int print_results(char output[], struct WordFreq ** wfpp, int size){
